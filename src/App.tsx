...
export default function App() {
  ...
  return (
    <div className="orbit-dark min-h-screen w-full overflow-x-hidden relative">
      ...
      <CelestialEffects mode={celestialMode} testMode={screen === 'decks'} />
...
function NebulaBackground() {
  return (
    <div className="fixed inset-0 pointer-events-none overflow-hidden" style={{ zIndex: 0 }}>
      ...
      {/* Violett — startet oben-links, weit außerhalb des Screens, driftet rechts-unten */}
      <div style={{
        position: 'absolute',
        width: '145%', height: '75%',
        left: '-55%', top: '-30%',
        background: 'radial-gradient(ellipse at 60% 50%, rgba(124,58,237,0.22) 0%, rgba(124,58,237,0.08) 40%, rgba(124,58,237,0.02) 70%, transparent 100%)',
        filter: 'blur(70px)',
        animation: 'nebula-violet 85s ease-in-out infinite',
        animationDelay: '0s',
      }} />

      {/* Cyan — startet unten-rechts außerhalb, driftet diagonal links-oben */}
      <div style={{
        position: 'absolute',
        width: '130%', height: '70%',
        right: '-50%', bottom: '-20%',
        background: 'radial-gradient(ellipse at 40% 50%, rgba(6,182,212,0.18) 0%, rgba(6,182,212,0.06) 40%, rgba(6,182,212,0.01) 70%, transparent 100%)',
        filter: 'blur(70px)',
        animation: 'nebula-cyan 110s ease-in-out infinite',
        animationDelay: '-38s',
      }} />

      {/* Pink — startet rechts außerhalb der Mitte, driftet horizontal nach links */}
      <div style={{
        position: 'absolute',
        width: '110%', height: '55%',
        right: '-55%', top: '30%',
        background: 'radial-gradient(ellipse at 55% 45%, rgba(236,72,153,0.16) 0%, rgba(236,72,153,0.05) 40%, rgba(236,72,153,0.01) 70%, transparent 100%)',
        filter: 'blur(70px)',
        animation: 'nebula-pink 140s ease-in-out infinite',
        animationDelay: '-65s',
      }} />
