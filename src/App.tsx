        ...
        <AnimatePresence>
          {!isLearning && (
            ...
            >
              ...
              {/* Profil-Chip rechts im Header */}
              {settings.profile ? (
                <motion.button
                  whileTap={{ scale: 0.92 }}
                  onClick={() => setProfileModal({ mode: 'edit' })}
                  className="flex items-center gap-2 bg-white/6 hover:bg-white/10 border border-white/10 rounded-full px-3 py-1.5 transition-all"
                >
                  <span className="text-lg leading-none">{AVATARS[settings.profile.avatarId]}</span>
                  <span className="text-xs font-bold text-white/80 max-w-[72px] truncate">{settings.profile.callsign}</span>
                  <span className="text-[9px] font-black text-violet-400">LV{levelData.level}</span>
                </motion.button>
              ) : (
                <div className="w-8" />
              )}
