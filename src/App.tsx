...
import { Rocket, LayoutGrid, BarChart3, Brain, ShieldCheck, Flame, Gamepad2 } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import { CelestialEffects, type CelestialMode } from './components/CelestialEffects';
import { Sun, Moon } from 'lucide-react';

type Screen = 'decks' | 'launch' | 'learn' | 'quiz' | 'cloze' | 'train' | 'stats' | 'manage';
...
export default function App() {
  ...
  const isLearning = screen === 'learn' || screen === 'launch' || screen === 'manage' || screen === 'quiz' || screen === 'cloze';

  const celestialMode: CelestialMode =
    screen === 'launch' ? 'epic'
    : screen === 'learn'  ? 'focus'
    : (screen === 'quiz' || screen === 'cloze') ? 'active'
    : 'ambient';
        ...
        <main className="flex-1">
          <AnimatePresence mode="wait">
            {screen === 'manage' && manageDeckId ? (
              ...
              <motion.div key="train" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ duration: 0.25 }}>
                <TrainingMenuScreen onSelectMode={(mode) => setScreen(mode)} />
              </motion.div>
            ) : screen === 'quiz' ? (
              <motion.div key="quiz" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ duration: 0.25 }}>
                <QuizScreen onClose={() => setScreen('train')} />
              </motion.div>
            ) : screen === 'cloze' ? (
              <motion.div key="cloze" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ duration: 0.25 }}>
                <ClozeScreen onClose={() => setScreen('train')} />
              </motion.div>
            ) : screen === 'launch' ? (
        ...
        {!isLearning && (
          ...
          >
            <div className="flex items-center justify-around w-full max-w-2xl mx-auto px-2 py-2">
              ...
              <NavDivider />
              <NavButton icon={<Brain size={22} />} label="Train" active={screen === 'train'} onClick={() => setScreen('train')} />
              <NavDivider />
