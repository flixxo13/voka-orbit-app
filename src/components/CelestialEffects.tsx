...
// ─── Lens Flare (Light Mode Highlight) ───────────────────────────────────────
// Geometrisch-präziser Sonnenreflex: 3 konzentrische Ringe + prismatischer Streifen.
// Position (x) wird einmalig pro Theme-Wechsel bestimmt (15%, 50%, oder 85% von links).
function LensFlare({ x }: { x: number }) {
  const T = 1.8;
  const fade = { duration: T, times: [0, 0.12, 0.82, 1.0], ease: 'easeInOut' as const };
  ...
  return (
    ...
    >
      <defs>
        {/* Prismatischer Streifen: subtile Regenbogenfarben */}
        <linearGradient id="flare-prism" x1="0%" y1="0%" x2="100%" y2="0%">
          <stop offset="0%"   stopColor="rgba(255,255,255,0)"   />
          <stop offset="20%"  stopColor="rgba(200,220,255,0.12)"/>
          <stop offset="40%"  stopColor="rgba(220,255,230,0.10)"/>
          <stop offset="60%"  stopColor="rgba(255,240,200,0.10)"/>
          <stop offset="80%"  stopColor="rgba(255,210,220,0.12)"/>
      ...
      {/* Prismatischer Querstreifen (leicht diagonal) */}
      <rect x={x - 55} y={-2} width={110} height={3.5}
        fill="url(#flare-prism)"
        transform={`rotate(-8, ${x}, 0)`}
        opacity={0.7}
      />

      {/* Ring 1: groß, sehr transparent */}
      <circle cx={x} cy={4} r={16} fill="none"
        stroke="rgba(255,255,255,0.12)" strokeWidth={1.2}
        filter="url(#flare-glow)"
      />
      {/* Ring 2: mittel */}
      <circle cx={x} cy={4} r={8} fill="none"
        stroke="rgba(220,235,255,0.22)" strokeWidth={0.9}
      />
      {/* Ring 3: klein, hellster */}
      <circle cx={x} cy={4} r={3.5} fill="none"
        stroke="rgba(255,255,255,0.38)" strokeWidth={0.6}
      />
      {/* Kern: heller Punkt */}
      <circle cx={x} cy={4} r={1.2}
        fill="rgba(255,255,255,0.70)" filter="url(#flare-glow)"
      />
    </motion.svg>
  );
}
...
export const CelestialEffects = ({
  ...
  const starCount = isLight ? 8 : (mode === 'focus' ? 12 : 30);
  ...
  const stars = useMemo(() =>
    Array.from({ length: starCount }).map((_, i) => {
      ...
      const size = isLight
        ? depth * 0.6 + 0.3   // light: 0.3–0.9px, sehr zart, schimmern durch
        : mode === 'focus'
          ? depth * 0.7 + 0.4
          : depth * 1.0 + 0.4;
      ...
      return {
        ...
        depth,
        baseOpacity: isLight
          ? 0.05 + depth * 0.15   // light: sehr gedämpft durch Atmosphärenlicht
  ...
  // ── Lens Flare (nur Light Mode) ────────────────────────────────────────────
  // Position wird einmalig beim Mount bestimmt (bleibt stabil bis Theme-Wechsel)
  const flareX = useRef(
    (() => { const opts = [15, 50, 85]; return opts[Math.floor(Math.random() * 3)]; })()
  );
  const [lensFlare, setLensFlare] = useState(false);

  useEffect(() => {
    if (!isLight || mode !== 'ambient') return;
    let running = true;
    const cycle = async () => {
      ...
      while (running) {
        ...
        if (!running) break;
        setLensFlare(false);
        await new Promise(r => setTimeout(r, 40000 + Math.random() * 30000));
      }
    };
    cycle();
    return () => { running = false; };
  ...
  return (
    <div className="fixed inset-0 pointer-events-none overflow-hidden z-[1]">
      ...
      {isLight && (
        <AnimatePresence>
          {lensFlare && (
            <LensFlare key="lf" x={flareX.current} />
          )}
        </AnimatePresence>
      )}
