...
export function OrbitHintSystem({
  ...
  useEffect(() => {
    const handleMotion = (e: DeviceMotionEvent) => {
      ...
      if (now - lastShakeRef.current.time > 100) {
        const diffTime = now - lastShakeRef.current.time;
        const speed = Math.abs(current.x + (current.y || 0) + (current.z || 0) - lastShakeRef.current.x - lastShakeRef.current.y - lastShakeRef.current.z) / diffTime * 10000;
        
        if (speed > 40 && !isDone && animatingOrbId === null) {
          // Shake detected!
  ...
  }, [isDone, animatingOrbId, soundEnabled]);
