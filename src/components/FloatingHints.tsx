...
/* ============================================================
   ...
   ============================================================ */
const ORB_COLORS = [
  { from: '#7C3AED', to: '#4F46E5', glow: '0 0 20px 4px rgba(124,58,237,0.6)' }, // Purple -> Indigo
  { from: '#06B6D4', to: '#0891B2', glow: '0 0 20px 4px rgba(6,182,212,0.6)' }, // Cyan
  { from: '#EC4899', to: '#BE185D', glow: '0 0 20px 4px rgba(236,72,153,0.6)' }, // Pink
  { from: '#F59E0B', to: '#D97706', glow: '0 0 20px 4px rgba(245,158,11,0.6)' }, // Amber
  { from: '#A3E635', to: '#059669', glow: '0 0 20px 4px rgba(163,230,53,0.6)' }, // Lime -> Emerald
  { from: '#60A5FA', to: '#2563EB', glow: '0 0 20px 4px rgba(96,165,250,0.6)' }, // Blue
  { from: '#A78BFA', to: '#4338CA', glow: '0 0 20px 4px rgba(167,139,250,0.6)' }, // Violet
  { from: '#F87171', to: '#B91C1C', glow: '0 0 20px 4px rgba(248,113,113,0.6)' }, // Coral Red
  { from: '#2DD4BF', to: '#0D9488', glow: '0 0 20px 4px rgba(45,212,191,0.6)' }, // Tropical Teal
  { from: '#FB923C', to: '#C2410C', glow: '0 0 20px 4px rgba(251,146,60,0.6)' }, // Fire Orange
] as const;
...
function buildPool(word: string, nextIdx: number): OrbData[] {
  ...
  // Zufällige Farbauswahl aus allen verfügbaren ORB_COLORS
  const colorIndices = shuffle(Array.from({ length: ORB_COLORS.length }, (_, k) => k));
              ...
              >
                ...
                >
                  ...
                  <motion.div
                    ...
                    className={`absolute w-full h-full rounded-full flex items-center justify-center ${sunState !== 'collapsing' ? 'sun-aura' : ''}`}
                    style={sunState === 'collapsing' ? {
                      boxShadow: '0 0 20px #000'
                    } : {}}
                  >
                    {/* Inner 3D Textures (Clipped by sun-sphere) */}
                    {sunState !== 'collapsing' ? (
                      <div className="sun-sphere w-full h-full absolute">
                        <div className="sun-layer-base" />
                        <div className="sun-layer-detail" />
                      </div>
                    ) : (
                      <div className="w-full h-full rounded-full" style={{ background: '#000', boxShadow: 'inset 0 0 20px #000' }} />
                    )}
              ...
              return (
                ...
                >
                  <PlanetVibe
                    ...
                    isWrong={false}
                    zone={orb.zone}
                    sunPhase={sunState === 'idle' ? 0 : sunState === 'forming' ? 1 : sunState === 'full' ? 2 : 3}
                  />
                  {/* Comet trail */}
                  <div style={{
