...
export function OrbitHintSystem({
  ...
  const handleCenterTap = useCallback(() => {
    // Block while animating, sucking, done, or uses exhausted
    if (isDone || suckedOrbId !== null || animatingOrbIdRef.current !== null || bhUsesRemaining <= 0) return;
    ...
    <div className="flex flex-col w-full gap-3 select-none">
      ...
      <div className="relative w-full" style={{ minHeight: 200 }}>
        ...
        >
          ...
          {/* Remaining uses dots – anchored below center */}
          <div className="absolute flex gap-1" style={{ top: '62%' }}>
            {[0, 1, 2].map(i => (
              <div
                key={i}
                className="w-1.5 h-1.5 rounded-full transition-all duration-300"
                style={{
                  background: i < bhUsesRemaining
                    ? 'rgba(139,92,246,0.7)'
                    : 'rgba(255,255,255,0.1)',
                  boxShadow: i < bhUsesRemaining ? '0 0 4px rgba(139,92,246,0.6)' : 'none',
                }}
              />
            ))}
          </div>
