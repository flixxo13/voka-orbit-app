...
export function OrbitHintSystem({
  ...
  /* ── Black Hole ── */
  useEffect(() => {
    if (bhCharge > 0 && bhCharge < 100) {
      const timer = setTimeout(() => {
        setBhCharge(prev => Math.max(0, prev - 3));
      }, 100);
  ...
  const handleCenterTap = () => {
    if (isDone || suckedOrbId || bhCharge >= 100) return;
    
    setBhCharge(prev => {
      const next = prev + 15; // Requires ~7 rapid taps
      if (next >= 100) {
        vibrateShake();
        playShuffleSound(soundEnabled);
        
        const distractor = pool.find(orb => 
          !cleanWord.includes(orb.letter) && !deadOrbIds.includes(orb.id) && orb.id !== animatingOrbId
        );
        
        if (distractor) {
          setSuckedOrbId(distractor.id);
          setTimeout(() => {
            setDeadOrbIds(d => [...d, distractor.id]);
            setSuckedOrbId(null);
            setBhCharge(0);
          }, 600); // 600ms animation
        } else {
          setTimeout(() => setBhCharge(0), 500); // Reset if no distractor
        }
      }
  ...
  return (
    <div className="flex flex-col w-full gap-3 select-none">
      ...
      <div className="relative w-full" style={{ minHeight: 200 }}>
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none" style={{ perspective: '800px' }}>
          ...
          <motion.div 
            ...
            style={{
              ...
              borderBottomColor: 'rgba(139,92,246,0.02)',
        ...
        >
          <div 
            className="absolute w-32 h-32 rounded-full cursor-pointer touch-manipulation z-20"
            onClick={handleCenterTap}
          />
          {bhCharge > 0 && (
            <motion.div
              initial={{ scale: 0 }}
              animate={{ scale: Math.max(0.1, bhCharge / 100) }}
              transition={{ duration: 0.1, ease: 'easeOut' }}
              className="absolute w-16 h-16 rounded-full bg-black shadow-[0_0_30px_rgba(139,92,246,0.8),inset_0_0_15px_rgba(6,182,212,0.6)] border border-purple-500/50 pointer-events-none"
            />
          )}
        </div>
          ...
          {pool.map(orb => {
            if (orb.id === animatingOrbId) {
              ...
              return (
                <motion.div
                  key={orb.id}
                  initial={{ scale: 1, opacity: 1,  left: ZONES[orb.zone].x, top: ZONES[orb.zone].y, x: 0 }}
                  animate={{ scale: 0.25, opacity: 0, y: 180, left: '50%', x: '-50%' }}
            ...
            if (orb.id === suckedOrbId) {
              return (
                <motion.div
                  ...
                  animate={{ scale: 0, opacity: 0, left: '50%', top: '50%', x: '-50%', y: '-50%', rotate: 720 }}
