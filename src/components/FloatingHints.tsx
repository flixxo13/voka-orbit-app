  ...
  return (
    <motion.div
      animate={
        ...
        isWarning ? { scale: [1, 1.1, 1], x: [-2, 2, -2, 2, 0], borderColor: ['rgba(6,182,212,0.5)', 'rgba(236,72,153,0.8)', 'rgba(6,182,212,0.5)'] } :
        isCombo ? { scale: [1, 1.05, 1], borderColor: ['rgba(6,182,212,0.5)', 'rgba(251,191,36,1)', 'rgba(6,182,212,0.5)'] } :
        { scale: 1 }
      }
      transition={
        isRevealed ? { duration: 0.3, ease: "easeOut" } :
        isWarning ? { duration: 0.4 } :
      ...
      className={`
        ...
        ${isRevealed
          ...
          : isNext
          ? `bg-cyan-500/10 text-white/20 ${
              isWarning ? 'border-pink-500/70 shadow-[0_0_10px_rgba(236,72,153,0.4)]' 
            : isCombo ? `border-amber-400 shadow-[0_0_${Math.min(10 + consecutiveCorrect * 3, 20)}px_rgba(251,191,36,0.6)]` 
            : isDanger ? 'border-pink-400/50 shadow-[0_0_8px_rgba(236,72,153,0.2)]' 
            : 'border-cyan-500/40 shadow-[0_0_8px_rgba(6,182,212,0.2)]'
            }`
          ...
          {pool.map(orb => {
            ...
            return (
              <motion.div 
                layout
                key={orb.id}
                className={isDead ? '' : floatClasses[orb.zone % 4]}
                style={{
                  position: 'absolute',
                  left: zone.x,
                  top: zone.y,
