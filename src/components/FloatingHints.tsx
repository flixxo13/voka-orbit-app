...
export function OrbitHintSystem({
  ...
  useEffect(() => {
    ...
    const handleMotion = (e: DeviceMotionEvent) => {
      ...
      if (now - lastShakeRef.current.time > 100) {
        ...
        // Threshold for a strong movement
        if (deltaX + deltaY + deltaZ > 30 && !isDone && animatingOrbId === null) {
            if (now - lastShakeTime > 800) {
                shakeCount = 0; // reset if too much time passed between shakes
            }
            shakeCount++;
            lastShakeTime = now;
            
            // Require 4 strong shakes back and forth
  ...
  }, [isDone, animatingOrbId, soundEnabled, isShaking]);
    ...
    <div className="flex flex-col w-full gap-3 select-none">
      ...
      <div className="relative w-full" style={{ minHeight: 200 }}>
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          ...
          <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,_rgba(124,58,237,0.08)_0%,_transparent_70%)] opacity-80" />
              ...
              >
                <motion.div
                  animate={isShaking && !isDead ? {
                    x: (orb.zone === 0 || orb.zone === 2) ? -25 : 25,
                    y: (orb.zone === 0 || orb.zone === 1) ? -25 : 25,
                    scale: 1.15
                  } : { x: 0, y: 0, scale: 1 }}
