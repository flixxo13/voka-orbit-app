...
export function OrbitHintSystem({
  ...
  // Sun Easter Egg State
  const [sunState, setSunState] = useState<'idle' | 'forming' | 'full' | 'exploding' | 'collapsing'>('idle');
  const [sunLevel, setSunLevel] = useState(0);
  const [isSunEligible, setIsSunEligible] = useState(true);
  ...
  const handleCenterTap = useCallback(() => {
    ...
    // Block if Sun is active or animating
    if (sunState !== 'idle') return;
  ...
  }, [isDone, suckedOrbId, bhUsesRemaining, cleanWord, soundEnabled, sunState]);
      ...
      if (now - lastShakeRef.current.time > 100) {
        ...
        if (deltaX + deltaY + deltaZ > 25 && !isDone && animatingOrbId === null && shufflesRemaining > 0) {
            ...
            if (shakeCount >= 4 && !isShaking) {
                ...
                if (distractor) {
                   setDeadOrbIds(prev => [...prev, distractor.id]);
                }
                
                // Drift out for 1.5s, then return
                setTimeout(() => {
                  setIsShaking(false);
                }, 1500);
            }
  ...
  const handleTap = useCallback((orb: OrbData) => {
    ...
    if (orb.letter === expected) {
      ...
      // SUN LOGIC (CORRECT)
      if (isSunEligible) {
        if (newCorrect === totalLetters && totalLetters >= 3) {
          if (sunTimerRef.current) clearTimeout(sunTimerRef.current);
          setSunState('full');
          setSunLevel(newCorrect - 3);
      ...
      // SUN LOGIC (WRONG)
      if (isSunEligible && consecutiveCorrect >= 3) {
        if (sunTimerRef.current) clearTimeout(sunTimerRef.current);
        if (nextBlankIdx === totalLetters - 1) {
          setSunState('exploding');
          sunTimerRef.current = setTimeout(() => {
            setSunState('idle');
            setSunLevel(0);
          }, 1500);
        } else {
          setSunState('collapsing');
          sunTimerRef.current = setTimeout(() => {
            setSunState('idle');
            setSunLevel(0);
          }, 1000);
      ...
      setIsSunEligible(false); // Fehler gemacht -> Sonne kommt nicht mehr wieder!
  ...
  /* ── Sun Proportional Scale Calculation ── */
  const sunMaxSteps = Math.max(1, totalLetters - 3);
  const sunProgress = Math.min(1, Math.max(0, sunLevel / sunMaxSteps));
  const currentSunScale = totalLetters <= 3 ? 1.2 : 0.3 + sunProgress * (1.2 - 0.3);
        ...
        >
          ...
          <AnimatePresence>
            {sunState !== 'idle' && (
              ...
              >
                {/* Outer Container: Smooth Base Growth */}
                <motion.div
                  initial={{ scale: 0, opacity: 0 }}
                  animate={
                    sunState === 'exploding' ? { scale: [currentSunScale, currentSunScale * 3.5], opacity: [1, 0], filter: ['brightness(1)', 'brightness(4)'] } :
                    sunState === 'collapsing' ? { scale: [currentSunScale, 0.1, 0], opacity: [1, 1, 0] } :
                    { scale: currentSunScale, opacity: 1 } // smooth base growth for forming and full
                  }
                ...
                >
                  {/* Inner Container: Pulsing and Styling */}
                  <motion.div
                    animate={
                      sunState === 'full' 
                    ...
                    transition={
                      sunState === 'forming' ? { duration: 2.5, repeat: Infinity, ease: "easeInOut" } :
                      { duration: 0.5 }
                    }
                    className="absolute w-full h-full rounded-full flex items-center justify-center"
                    style={{
                      background: sunState === 'collapsing' 
                        ? '#000' 
                        : 'radial-gradient(circle at center, #FFFFFF 0%, #FEF08A 15%, #F59E0B 45%, rgba(180,83,9,0.8) 75%, transparent 100%)',
                      boxShadow: sunState === 'collapsing' 
                        ? '0 0 20px #000, inset 0 0 20px #000' 
                        : '0 0 40px 5px rgba(245,158,11,0.8), 0 0 80px 15px rgba(251,191,36,0.4)',
                    }}
                  >
                     {/* ── Corona Layer 1: Fast spinning flares, masked to fade outward ── */}
                     {sunState !== 'collapsing' && (
                       <motion.div
                         ...
                         className="absolute w-[320%] h-[320%] pointer-events-none"
                         style={{
                           // conic paints the ray pattern; mask fades it radially outward → no blur needed!
                           background: 'conic-gradient(from 0deg, transparent 0%, rgba(253,224,71,0.9) 6%, transparent 18%, rgba(245,158,11,0.7) 38%, transparent 52%, rgba(253,224,71,0.6) 68%, transparent 86%)',
                           maskImage: 'radial-gradient(circle, transparent 18%, rgba(0,0,0,0.6) 35%, rgba(0,0,0,0.3) 60%, transparent 100%)',
                           WebkitMaskImage: 'radial-gradient(circle, transparent 18%, rgba(0,0,0,0.6) 35%, rgba(0,0,0,0.3) 60%, transparent 100%)',
                           mixBlendMode: 'screen',
                         }}
                       />
                     )}

                     {/* ── Corona Layer 2: Slow counter-spin, wider & softer ── */}
                     {sunState !== 'collapsing' && (
                       <motion.div
                         ...
                         className="absolute w-[240%] h-[240%] pointer-events-none"
                         style={{
                           background: 'conic-gradient(from 90deg, transparent 0%, rgba(255,200,50,0.7) 8%, transparent 22%, rgba(255,140,0,0.5) 50%, transparent 70%, rgba(255,220,80,0.4) 85%, transparent 95%)',
                           maskImage: 'radial-gradient(circle, transparent 22%, rgba(0,0,0,0.5) 38%, rgba(0,0,0,0.2) 65%, transparent 100%)',
                           WebkitMaskImage: 'radial-gradient(circle, transparent 22%, rgba(0,0,0,0.5) 38%, rgba(0,0,0,0.2) 65%, transparent 100%)',
                     ...
                     {/* Full Power blinding core */}
                     {sunState === 'full' && (
                       <motion.div
                         animate={{ scale: [1, 1.4, 1], opacity: [0.7, 1, 0.7] }}
                         transition={{ duration: 0.9, repeat: Infinity, ease: 'easeInOut' }}
                         className="absolute w-[90%] h-[90%] rounded-full bg-white"
                         style={{ boxShadow: '0 0 24px 12px rgba(255,255,255,0.9)' }}
                       />
                     )}
                  </motion.div>
                </motion.div>
                
                {/* Shockwave for Explosion */}
                {sunState === 'exploding' && (
                  <motion.div
                    initial={{ scale: currentSunScale, opacity: 1, borderWidth: 8 }}
                    animate={{ scale: currentSunScale * 5, opacity: 0, borderWidth: 0 }}
                    transition={{ duration: 0.8, ease: "easeOut" }}
                    className="absolute rounded-full border-white/80"
                    style={{ width: 56, height: 56 }}
                  />
                )}
              </motion.div>
