            ...
            {sunState !== 'idle' && (
              ...
              >
                ...
                <motion.div
                  ...
                  animate={
                    sunState === 'full' 
                      ? { scale: Math.min(1 + sunLevel * 0.25, 2.5) * 1.5, opacity: 1, filter: 'brightness(2)' }
                      : sunState === 'exploding'
                      ? { scale: [Math.min(1 + sunLevel * 0.25, 2.5), Math.min(1 + sunLevel * 0.25, 2.5) * 4], opacity: [1, 0], filter: ['brightness(1)', 'brightness(3)'] }
                      : sunState === 'collapsing'
                      ? { scale: [Math.min(1 + sunLevel * 0.25, 2.5), 0.1, 0], opacity: [1, 1, 0], backgroundColor: ['#FDE047', '#000000', '#000000'] }
                      : { scale: Math.min(1 + sunLevel * 0.25, 2.5), opacity: 1 } // forming
                  }
                  exit={{ scale: 0, opacity: 0 }}
                  transition={
                    sunState === 'exploding' ? { duration: 0.8, ease: "easeOut" } :
                    sunState === 'collapsing' ? { duration: 0.8, ease: "anticipate" } :
                  ...
                  style={{
                    width: 70, height: 70,
                    background: sunState === 'collapsing' ? undefined : 'radial-gradient(circle at 30% 30%, #FFFFFF 0%, #FDE047 30%, #F59E0B 70%, #B45309 100%)',
                    boxShadow: sunState === 'collapsing' ? 'inset 0 0 20px #000' : '0 0 40px rgba(245,158,11,0.6), inset 0 0 15px rgba(255,255,255,0.8)',
                  }}
                >
                   {/* Rays / Corona */}
                   {sunState !== 'collapsing' && (
                     <motion.div
                       ...
                       className="absolute w-[200%] h-[200%] mix-blend-screen opacity-70"
                       style={{
                         background: 'conic-gradient(from 0deg, transparent 0%, rgba(253,224,71,0.6) 15%, transparent 35%, rgba(245,158,11,0.6) 65%, transparent 85%)'
                       }}
                     />
                   )}
                   ...
                   {sunState === 'full' && (
                     <motion.div
                       ...
                       className="absolute w-[80%] h-[80%] rounded-full bg-white blur-md"
                     />
                   )}
                </motion.div>
                ...
                {sunState === 'exploding' && (
                  <motion.div
                    initial={{ scale: Math.min(1 + sunLevel * 0.25, 2.5), opacity: 1, borderWidth: 8 }}
                    animate={{ scale: Math.min(1 + sunLevel * 0.25, 2.5) * 6, opacity: 0, borderWidth: 0 }}
