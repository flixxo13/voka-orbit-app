...
export function GradeFeedback({ grade, xpDelta, onDone }: GradeFeedbackProps) {
  ...
  return (
    ...
    >
      ...
      {Math.abs(xpDelta) > 0 && (
        ...
        >
          <Zap size={12} className={`${xpDelta > 0 ? 'text-amber-400' : 'text-red-400'} fill-current`} />
          <span className={`text-sm font-black ${xpDelta > 0 ? 'text-amber-300' : 'text-red-300'}`}>
            {xpDelta > 0 ? '+' : '-'}{Math.abs(xpDelta)} XP
          </span>
        </motion.div>
      )}
