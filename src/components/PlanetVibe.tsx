...
export function PlanetVibe({
  ...
  useEffect(() => {
    const handleOrientation = (e: DeviceOrientationEvent) => {
      ...
      setTiltX(gamma * 0.08); // slightly increased parallax
      setTiltY(beta * 0.08);
    };
  ...
  // Default lighting (Top-Left)
  let lightX = 4, lightY = 4;
  let shadowX = -6, shadowY = -6;

  // Target lighting (Center - towards the sun)
  let targetLightX = 4, targetLightY = 4;
  let targetShadowX = -6, targetShadowY = -6;

  if (zone === 0) {
    targetLightX = -4; targetLightY = -4;
    targetShadowX = 6; targetShadowY = 6;
  } else if (zone === 1) {
    targetLightX = 4; targetLightY = -4;
    targetShadowX = -6; targetShadowY = 6;
  } else if (zone === 2) {
  ...
  // Calculate interpolation factor based on sunPhase
  // 0: default, 1: 50% center, 2: 80% center, 3: 100% center
  const phaseFactor = sunPhase === 0 ? 0 : sunPhase === 1 ? 0.5 : sunPhase === 2 ? 0.8 : 1.0;

  const currentLightX = lightX + (targetLightX - lightX) * phaseFactor;
  const currentLightY = lightY + (targetLightY - lightY) * phaseFactor;
  const currentShadowX = shadowX + (targetShadowX - shadowX) * phaseFactor;
  const currentShadowY = shadowY + (targetShadowY - shadowY) * phaseFactor;

  const baseStyle = {
    '--planet-base-color': colorFrom,
    '--planet-color-to': colorTo,
    '--light-x': `${currentLightX}px`,
    '--light-y': `${currentLightY}px`,
    '--shadow-x': `${currentShadowX}px`,
    '--shadow-y': `${currentShadowY}px`,
  ...
  return (
    ...
    >
      ...
      {/* The 3D Sphere */}
      <div 
        className={`planet-sphere planet-type-${type}`}
      >
