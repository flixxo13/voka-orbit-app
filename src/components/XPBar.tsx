...
export function XPToast({ amount, label, onDone }: XPToastProps) {
  ...
  return (
    ...
    >
      ...
      >
        <Zap size={13} className={`fill-current ${isBonus ? 'text-violet-300' : 'text-amber-400'}`} />
        <span className={`text-sm font-black ${isBonus ? 'text-violet-300' : 'text-amber-400'}`}>
