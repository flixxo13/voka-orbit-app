...
export function XPToast({ amount, label, isNegative, onDone }: XPToastProps) {
  ...
  const isSmallReward = amount < 10 && !isNegative;
  ...
  return (
    ...
    >
      <motion.div
        animate={{
          boxShadow: isBonus
            ? ['0 0 0px rgba(124,58,237,0)', '0 0 18px rgba(124,58,237,0.6)', '0 0 0px rgba(124,58,237,0)']
            : isNegative
      ...
      >
        <Zap size={13} className={`fill-current ${isBonus ? 'text-violet-300' : isNegative ? 'text-red-400' : isSmallReward ? 'text-emerald-400' : 'text-amber-400'}`} />
        <span className={`text-sm font-black ${isBonus ? 'text-violet-300' : isNegative ? 'text-red-400' : isSmallReward ? 'text-emerald-400' : 'text-amber-400'}`}>
