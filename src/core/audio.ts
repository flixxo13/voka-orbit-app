...
export async function playCorrectSound(enabled: boolean) {
  if (!enabled) return;
  const ctx = await getContext();
...
export async function playWrongSound(enabled: boolean) {
  if (!enabled) return;
  const ctx = await getContext();
  const osc = ctx.createOscillator();
  const gain = ctx.createGain();
...
export async function playShuffleSound(enabled: boolean) {
  if (!enabled) return;
  const ctx = await getContext();
  
  // A quick "swish" noise using oscillator FM or fast frequency sweep
  const osc = ctx.createOscillator();
  const gain = ctx.createGain();
