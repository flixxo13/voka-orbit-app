...
export async function getDueCards(deckId?: number, limit: number = 20) {
  ...
  const now = Date.now();
  const allCardIds = allCards.map(c => c.id!).filter((id): id is number => id !== undefined);
  
  let relevantReviews: Review[] = [];
  if (allCardIds.length > 0) {
    relevantReviews = await db.reviews.where('cardId').anyOf(allCardIds).toArray();
  }
  
  const dueReviews = relevantReviews.filter(r => r.nextDueAt <= now);
  const dueCardIds = new Set(dueReviews.map(r => r.cardId));
  const reviewedCardIds = new Set(relevantReviews.map(r => r.cardId));
