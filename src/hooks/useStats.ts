...
export function useStats(deckId?: number) {
  const stats = useLiveQuery(async () => {
    ...
    const reviewedCardIdsSet = new Set(deckReviews.map(r => r.cardId));
    const totalReviewed = reviewedCardIdsSet.size;
    const newCards = Math.max(0, totalCards - totalReviewed);

    // Latest review per card for learning stages
    const latestReviewPerCard = new Map<number, typeof deckReviews[0]>();
    ...
    const latestReviews = Array.from(latestReviewPerCard.values());
