...
export function CardManagementScreen({ deckId, onBack }: { deckId: number, onBack: () => void }) {
  ...
  const handleDeleteCard = async (id: number) => {
    ...
    await db.quizReviews.where('cardId').equals(id).delete();
      ...
      <AnimatePresence>
        {confirmDelete && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            ...
            >
              <h3 className="text-xl font-black text-white uppercase tracking-tight mb-4">Karte löschen</h3>
              <p className="text-slate-400 text-sm mb-6">Diese Karte wirklich löschen?</p>
              <div className="flex gap-3">
                <button 
                  onClick={() => setConfirmDelete(null)}
                  className="flex-1 py-3 rounded-xl text-slate-400 font-bold uppercase text-xs tracking-wider bg-white/5 hover:bg-white/10 transition-all"
                >
                  Abbrechen
                </button>
                <button 
                  onClick={() => handleDeleteCard(confirmDelete)}
                  className="flex-1 py-3 rounded-xl bg-red-500/20 hover:bg-red-500/30 text-red-400 font-black uppercase text-xs tracking-wider border border-red-500/30 transition-all"
                >
                  Löschen
                </button>
              </div>
