...
export function DeckScreen({ onSelectDeck, onManageCards, requireProfile }: {
  ...
  const handleDeleteDeck = async (id: number) => {
    const cardIds = (await db.cards.where('deckId').equals(id).toArray()).map(c => c.id!);
            ...
            {decks?.map((deck, i) => (
              ...
              >
                <PlanetDeckCard
                  ...
                  onDelete={() => setConfirmDelete({ id: deck.id!, name: deck.name })}
      ...
      {confirmDelete && (
        <ModalOverlay>
          <div className="flex justify-between items-center mb-2">
            <h3 className="text-xl font-black text-white uppercase tracking-tight">Deck löschen</h3>
          </div>
          <p className="text-white/60 text-sm mb-4">
            Deck „{confirmDelete.name}“ und alle Karten wirklich löschen?
          </p>
          <div className="flex gap-3 pt-1">
            <button onClick={() => setConfirmDelete(null)} className="flex-1 py-3.5 rounded-2xl text-white/40 font-bold text-xs uppercase tracking-wider bg-white/5">Abbrechen</button>
            <motion.button whileTap={{ scale: 0.95 }} onClick={() => handleDeleteDeck(confirmDelete.id)}
              className="flex-1 py-3.5 rounded-2xl bg-red-500/20 text-red-400 font-black text-xs uppercase tracking-wider border border-red-500/30">
              Löschen
            </motion.button>
          </div>
        </ModalOverlay>
