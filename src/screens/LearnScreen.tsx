...
import { ArrowLeft, Star, Rocket, Check, X, FastForward, Zap, Info } from 'lucide-react';

type LearnPhase = 'card' | 'hint';

interface PendingXP { amount: number; label: string; key: number; isNegative?: boolean; isCombo?: boolean; isWarning?: boolean; }
...
export function LearnScreen({ deckId, onBack }: { deckId?: number; onBack: () => void }) {
  ...
  const [hintXpDelta, setHintXpDelta] = useState(0);
  ...
  /* ── XP helpers ── */
  const showXP = useCallback((amount: number, label?: string, isNegative?: boolean, isCombo?: boolean, isWarning?: boolean) => {
    setPendingXP(prev => [...prev, { amount, label: label ?? '', key: xpKey, isNegative, isCombo, isWarning }]);
    setXpKey(k => k + 1);
  }, [xpKey]);
  ...
  const handleSpendXP = useCallback((amount: number, isWarning?: boolean): boolean => {
    ...
    if (ok) {
      showXP(amount, isWarning ? '3x Falsch!' : '', true, false, isWarning);
      setHintXpDelta(prev => prev - amount);
    }
    return ok;
  }, [spendXP, showXP]);
  ...
  const handleRevealComplete = useCallback(() => {
    setTimeout(() => {
      showXP(Math.abs(hintXpDelta), 'Bilanz', hintXpDelta < 0);
    }, 600);
    setIsFlipped(true);
    setPhase('card');
  }, [hintXpDelta, showXP]);
  ...
  const handleAbortHint = useCallback((cost: number) => {
    if (cost > 0) {
      ...
      setHintXpDelta(prev => prev - cost);
    }
    setTimeout(() => {
      showXP(Math.abs(hintXpDelta - cost), 'Bilanz', (hintXpDelta - cost) < 0);
    }, 600);
    setForceNochmal(true);
    setIsFlipped(true);
    setPhase('card');
  }, [spendXP, showXP, hintXpDelta]);
    ...
    return (
      ...
      >
        ...
        <div>
          ...
          >
            {hasCards
              ? 'Alle fälligen Vokabeln gelernt. Dein Orbit ist stabil. 🛸'
  ...
  return (
    <div className="flex flex-col gap-4 py-2 relative min-h-[80vh]">
      ...
      {/* XP Toasts */}
      <div className="absolute top-16 right-4 flex flex-col items-end gap-2 pointer-events-none z-50">
        <AnimatePresence>
          {pendingXP.map(p => (
            <XPToast key={p.key} amount={p.amount} label={p.label} isNegative={p.isNegative} isCombo={p.isCombo} isWarning={p.isWarning} onDone={() =>
