...
interface PendingXP { amount: number; label: string; key: number; isNegative?: boolean; }
...
export function LearnScreen({ deckId, onBack }: { deckId?: number; onBack: () => void }) {
  ...
  /* ── XP helpers ── */
  const showXP = useCallback((amount: number, label?: string, isNegative?: boolean) => {
    setPendingXP(prev => [...prev, { amount, label: label ?? '', key: xpKey, isNegative }]);
    setXpKey(k => k + 1);
  }, [xpKey]);

  const handleSpendXP = useCallback((amount: number): boolean => {
    const ok = spendXP(amount);
    if (ok) showXP(amount, '', true);
    return ok;
  }, [spendXP, showXP]);
  ...
  return (
    <div className="flex flex-col gap-4 py-2 relative min-h-[80vh]">
      ...
      <div className="absolute top-24 left-1/2 -translate-x-1/2 pointer-events-none z-50">
        <AnimatePresence>
          {pendingXP.map(p => (
            <XPToast key={p.key} amount={p.amount} label={p.label} isNegative={p.isNegative} onDone={() =>
            ...
            <AnimatePresence>
              {isFlipped && (
                <motion.div
                  ...
                  className={`w-full max-w-md grid ${hintsUsedThisCard > 0 ? 'grid-cols-2' : 'grid-cols-4'} gap-2`}
                >
                  {GRADE_BUTTONS.filter(btn => hintsUsedThisCard === 0 || btn.grade <= 2).map(btn => (
            ...
            <AnimatePresence>
              {!isFlipped && hintsUsedThisCard === 0 && (
                <motion.button
                  ...
                  className={`flex items-center gap-2 px-5 py-2.5 rounded-2xl glass-card border ${settings.xp >= XP_COSTS.HINT_ENTRY ? 'border-amber-500/20 text-amber-400' : 'border-white/10 text-white/30'} text-xs font-black uppercase tracking-wider`}
                >
                  💡 Hint Modus
        ...
        {phase === 'hint' && (
          ...
          >
            <OrbitHintSystem
              ...
              onGainXP={(amount) => { gainXP(amount); showXP(amount, ''); }}
