...
export function QuizScreen({ deckId, onClose }: QuizScreenProps) {
  ...
  return (
    <div className="flex flex-col h-full max-w-2xl mx-auto px-4 py-2 select-none">
      {/* Header & Progress */}
      <div className="mb-6 flex items-center gap-4 bg-white/[0.04] backdrop-blur-[20px] border border-white/[0.08] rounded-full p-2 pr-4 shadow-lg shadow-black/20">
        <div className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center shrink-0 border border-white/10 shadow-inner shadow-white/5">
        ...
        <button 
          onClick={onClose}
          className="w-8 h-8 rounded-full flex items-center justify-center bg-white/5 hover:bg-white/10 text-white/40 hover:text-white transition-all shrink-0"
        >
