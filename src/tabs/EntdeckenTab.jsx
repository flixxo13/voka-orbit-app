    ...
    <div style={styles.modalOverlay} onClick={onSchliessen}>
      <div style={styles.modal} onClick={e => e.stopPropagation()}>
        ...
        <div style={styles.vorschauListe}>
          {laden ? (
            <p style={{ color: '#94a3b8', padding: '1rem', textAlign: 'center', fontSize: '0.9rem' }}>
              Lädt...
            </p>
          ) : (
            vokabeln.map((v, i) => (
...
export default function EntdeckenTab({ einstellungen, setEinstellungen }) {
  const [index, setIndex] = useState(null)     // null = kein Modal offen
  const [listen, setListen] = useState([])
  const [laden, setLaden] = useState(true)
  const [toggleAnim, setToggleAnim] = useState(null) // animierte ID
  ...
  useEffect(() => {
    async function ladeIndex() {
      try {
        const res = await fetch('/listen/index.json')
        const daten = await res.json()
        setListen(daten)
      } catch { setListen([]) }
      setLaden(false)
  ...
  async function handleToggle(listenId) {
    ...
    setIndex(null) // Modal schließen
  }

  if (laden) {
    return (
      <div style={styles.container}>
        <p style={{ color: '#94a3b8', textAlign: 'center', padding: '3rem', fontSize: '0.9rem' }}>
          Lädt Listen...
        </p>
      </div>
    )
  ...
  // Listen nach Sprache gruppieren
  const nachSprache = {}
  listen.forEach(l => {
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.pageHeader}>
        ...
        <p style={styles.pageUntertitel}>
          Wähle welche Listen du lernen möchtest
        </p>
      </div>

      {/* Pro Sprache eine Gruppe */}
        ...
        <div key={sprache} style={styles.sprachGruppe}>
          ...
          {sprachListen.map((liste, i) => {
            ...
            return (
              <div
                key={liste.id}
                onClick={() => setIndex(istGlobal)}
                style={{
                  ...styles.listenKarte,
                  borderColor: istAktiv ? '#a78bfa' : '#e2e8f0',
                  background: istAktiv ? '#faf5ff' : 'white',
                  transform: animiert ? 'scale(0.97)' : 'scale(1)',
                  transition: 'all 0.2s ease',
                }}
              >
                {/* Links: Niveau + Beschreibung */}
                <div style={styles.listenKarteLinks}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                    <NiveauBadge niveau={liste.niveau} />
                    {istAktiv && (
                      <span style={styles.aktivBadge}>✓ Aktiv</span>
                    )}
                  </div>
                ...
                >
                  <div style={{
                    ...styles.toggleKnopf,
                    transform: istAktiv ? 'translateX(20px)' : 'translateX(2px)',
                  }} />
                </div>
      ...
      <div style={styles.infoBox}>
        <p style={styles.infoText}>
          💡 Tippe auf eine Liste für eine Vorschau der Vokabeln
        </p>
      </div>
...
const styles = {
  ...
  sprachName: {
    fontSize: '0.95rem', fontWeight: 700, color: '#334155', flex: 1
  },
  aktivZaehler: {
    fontSize: '0.75rem', color: '#7c3aed', fontWeight: 700,
    background: '#f5f3ff', padding: '2px 8px', borderRadius: 8
  },
  ...
  listenAnzahl: {
    fontSize: '0.75rem', color: '#94a3b8', margin: 0, fontWeight: 500
  },
  ...
  infoBox: {
    ...
    padding: '0.85rem 1rem', marginTop: 8,
  ...
  modalAnzahl: {
    fontSize: '0.8rem', color: '#94a3b8', margin: 0, fontWeight: 500
  },
  ...
  vorschauListe: {
    ...
    padding: '0.5rem', marginBottom: 16,
  ...
  vorschauUebersetzung: {
    color: '#475569', fontSize: '0.9rem', flex: 1
  },
  vorschauMehr: {
    textAlign: 'center', color: '#94a3b8',
    fontSize: '0.8rem', padding: '0.5rem', margin: 0
  },
